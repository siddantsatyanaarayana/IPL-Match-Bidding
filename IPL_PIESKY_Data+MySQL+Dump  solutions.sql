...
-- Dump completed
-- Write SQL queries to get data for following requirements:

-- 1.	Show the percentage of wins of each bidder in the order of highest to lowest percentage.
select ibd.bid_team,count(ibd.bid_status)/sum(ibp.no_of_bids)*100 as prcnt_win from IPL_BIDDING_DETAILS ibd join IPL_BIDDER_POINTS ibp
on ibd.bidder_id=ibp.bidder_id
 where bid_status='won'
 group by bid_team
 order by prcnt_win desc;
-- 2.	Display the number of matches conducted at each stadium with stadium name, city from the database.
select STADIUM_NAME,CITY,count(MATCH_ID)
from ipl_stadium a join 
ipl_match_schedule b on a.STADIUM_ID=b.STADIUM_ID
group by b.STADIUM_ID;
-- 3.	In a given stadium, what is the percentage of wins by a team which has won the toss?
select STADIUM_NAME,(count(c.MATCH_ID)/14)*100
from ipl_stadium a join 
ipl_match_schedule b on a.STADIUM_ID=b.STADIUM_ID
join ipl_match c on b.MATCH_ID=c.MATCH_ID
...
join ipl_team c on c.TEAM_ID=b.BID_TEAM
group by BID_TEAM;
-- 5.	Show the team id who won the match as per the win details.
select TEAM_ID
from ipl_team a join 
ipl_match b where a.TEAM_ID=b.MATCH_WINNER;
-- 6.	Display total matches played, total matches won and total matches lost by team along with its team name.
select TEAM_NAME,MATCHES_PLAYED,MATCHES_WON,MATCHES_LOST
from ipl_team a join 
ipl_team_standings b where a.TEAM_ID=b.TEAM_ID
group by TEAM_NAME;
...
select TEAM_NAME,count(PLAYER_ID) as no_all_rounders
from ipl_team a join 
ipl_team_players b on a.TEAM_ID=b.TEAM_ID
